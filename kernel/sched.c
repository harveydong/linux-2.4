...
static inline int goodness(struct task_struct * p, int this_cpu, struct mm_struct *this_mm)
{
	...
	if (p->policy == SCHED_OTHER) {
		...
		//进程上次在该cpu上运行过，那么权值就加一个常数*/
		if (p->processor == this_cpu)
			weight += PROC_CHANGE_PENALTY;
#endif

		/* .. and a slight advantage to the current MM */
		if (p->mm == this_mm || !p->mm) //表示进程是否需要切换内存,如果不需要,则权值+1.
			weight += 1;
		weight += 20 - p->nice; //这里是用户可控优先级nice
		goto out;
	}
...
//这个可以使进程休眠被唤醒，或者是schedule来调用，也即是说进程p变为可运行.
...
//该函数决定进程是否抢占某一cpu
	...
	if (can_schedule(p, best_cpu)) { //表示该task没有被执行，并且运行在best_cpu上执行.
		tsk = idle_task(best_cpu); //找到想要去的cpu上的task,
		if (cpu_curr(best_cpu) == tsk) {//如果当前处理器上的任务和idle_task中的一样，表示是idle.这里也可以是进程上次运行的cpu目前空闲. 这个是首选,如果不是,那么就接着往下走.
			int need_resched;
send_now_idle:
			/*
...
//比较cpu当前运行进程和p的抢先权,将具有最高抢先权的进程记录在taget_tsk中，
	for (i = 0; i < smp_num_cpus; i++) {
		 ...
		 * a problem.
//在所有空闲cpu中，找一个最近最少活跃的一个,也就是last_scheudle值最少的一个,这个会被实时更新的。所以该值越小，表明该cpu月没有活跃.
		 */
		...
		} else { //这里表示,cpu不空闲，但是当前运行进程优先级比p的优先级低,且差值最大
			if (oldest_idle == -1ULL) {
...
need_resched_back:
	prev = current;//当前进程
	this_cpu = prev->processor;//当前进程所运行的cpu

	if (in_interrupt()) //如果现在是中中断上下文中，那么是不允许调度的.
		goto scheduling_in_interrupt;

	release_kernel_lock(prev, this_cpu);

	/* Do "administrative" work here while we don't hold any locks */
	if (softirq_active(this_cpu) & softirq_mask(this_cpu)) //如果当前cpu上有软中断,那么先去处理软中断
		goto handle_softirq;
handle_softirq_back:
	 ...
	 */
	sched_data = & aligned_data[this_cpu].schedule_data;//拿到当前处理器数据

	spin_lock_irq(&runqueue_lock);

	/* move an exhausted RR process to be last.. */
	if (prev->policy == SCHED_RR)//当前进程是实时进程,那么其优先级最高了，需要特别处理
		goto move_rr_last;
move_rr_back:

	switch (prev->state) {
		case TASK_INTERRUPTIBLE:
			if (signal_pending(prev)) { //当该进程有信号需要处理时，那么还是把该进程置位RUNNING来参与调度
				prev->state = TASK_RUNNING;
				break;
			}
		default://到这里，表示prev进程不处于就绪态，并且没有信号需要处理,那么就把该进程从runqueue中拿掉
...
repeat_schedule:
	 ...
	 */
	next = idle_task(this_cpu); //达到该cpu上的idle进程,如果没有任务运行，那么就运行idle进程
	c = -1000;
	if (prev->state == TASK_RUNNING) //如果进程进来的状态是RUNNING的，那么表示其不是自愿的，那就是时间片用完了,如果是实时进程的话，那么其权重是最大的.如果是普通进程,那么其权重就是0.
		goto still_running;
...
still_running_back:
	list_for_each(tmp, &runqueue_head) {
		...
		if (can_schedule(p, this_cpu)) {
			...
			if (weight > c) //如果是实时进程，那么c就是最大的.如果是普通进程，那么c就是为0. 如果进程是由于需要等待的条件不满足，那么它在这里就不会在runqueue上了，不会参与调度了
				c = weight, next = p;
		}
	}

	/* Do we need to re-calculate counters? */
	if (!c) //某个时间片里面的进程都用完了其时间了，这是需要重新计算所有进程的时间片
		goto recalculate;
	/*
	...
	prepare_to_switch();
	{
		...
		if (!prev->mm) {//如果被换出去的是内核线程，那么需要把active_mm置为NULL.
			prev->active_mm = NULL;
			mmdrop(oldmm);
		}
	}
...
recalculate:
	{
		...
		for_each_task(p) //进程睡眠的交互式进程,其counter就给得多一点.
...
still_running:
	c = goodness(prev, this_cpu, prev->active_mm);//那么计算该进程的权重,
	next = prev;//从当前的开始
...
move_rr_last://实时进程的地方
	if (!prev->counter) {//很可能是实时进程的时间片用完了，那么就需要重新给他分配时间片，然后放回runqueue_head中去.
		prev->counter = NICE_TO_TICKS(prev->nice);//counter值变为０,则用priority,即静态优先级给其复制. 这里的counter是在动态的变化,所以称为动态优先调度.
		move_last_runqueue(prev);
	}
	goto move_rr_back; //那么如果不是时间片用完了呢,那么可能是实时进程需要的条件不满足，那么其进入的时候，它的状态就是INTERRUPT的，而不是RUNNING的.
